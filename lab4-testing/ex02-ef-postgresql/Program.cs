...
using Howest.lab2.ex02_ef_postgresql.Mappings;
using Howest.lab2.ex02_ef_postgresql.Repositories;
using Howest.lab2.ex02_ef_postgresql.Services;
using Howest.lab2.ex02_ef_postgresql.Validation;
...
app.MapGet("/persons/cached", (IPersonService service, IMemoryCache cache) =>
{
    var cachedPersons = cache.GetOrCreate("persons_cache", entry =>
    {
        entry.AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(5);
        return service.GetAllPersons();
    });
    return Results.Ok(cachedPersons);
});
